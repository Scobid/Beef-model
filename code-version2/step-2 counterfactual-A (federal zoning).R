...
## SCENARIO: FDM IN ALBERTA (ZONED: ONTARIO EXPORTS OKAY)
## ORIGINAL: TOR TOLHURST (OCT/2014)
## LAST UPDATE: TOR TOLHURST (JUN/2015)
...
for (i in qtr) {
  ...
  bkgd_2["q_sup steer",  ii] <- bkgd_2["q_sup steer",  ii] + max(round(assume2$dairy_fdr*fit(qq$bkgrd$dairy$data, logeq = F)$yhat)[i], 0)
  bkgd_2["q_sup heifer", ii] <- bkgd_2["q_sup heifer", ii] + max(round((1-assume2$dairy_fdr)*fit(qq$bkgrd$dairy$data, logeq = F)$yhat)[i], 0)
  ...
  if (exc_dem == 1){
    ...
    # demand by gender is now supply net replacements
    ...
    # there is no excess supply to export 
    ...
    bkgd_2["q_sup steer",  ii] <- bkgd_2["q_sup steer",  ii] + bkgd_2["q_rep steer",  ii] #+ bkgd_2["q_xus steer",  ii] 
    bkgd_2["q_sup heifer", ii] <- bkgd_2["q_sup heifer", ii] + bkgd_2["q_rep heifer", ii] #+ bkgd_2["q_xus heifer", ii]
    bkgd_2["q_sup total",  ii] <- bkgd_2["q_sup steer",  ii] + bkgd_2["q_sup heifer", ii]
  ...
  if (exc_dem == -1){
    ...
    # FINISHING GENDER -- for calculation
    ...
    # excess supply exported to U.S. (zoning & price-taking therefore no restrictions)
    bkgd_2["q_xus steer",  ii] <- bkgd_2["q_sup steer",  ii] - bkgd_2["q_dem steer",  ii]
    bkgd_2["q_xus heifer", ii] <- bkgd_2["q_sup heifer", ii] - bkgd_2["q_dem heifer", ii]
    bkgd_2["q_xus total",  ii] <- bkgd_2["q_sup total",  ii] - bkgd_2["q_dem total",  ii]
  ...
  rm(tmp1, den1, num1, num2, exc_dem)
