...
## SCENARIO: FDM IN WELLINGTON COUNTY
## FEDERAL AND PROVINCIAL BORDERS SHUT DOWN
## NO FEEDER MOVEMENT INTO THESE COUNTIES (5% of movement)
## CULL CATTLE (EQUIVALENT T0 25% OF CATTLE IN WELLINGTON COUNTY)
## % OF PROV. CATTLE: 1.6% CALVES, 2.7% STOCKERS, 1.6% NON-FED, 3.4% FINISHING
## 50% REDUCTION IN SLAUGHTER CAPACITY
...
assume2$stomp$calf <- 0.009 # cull 25% of calves in wellington county (=0.816%)
assume2$stomp$bkgd <- 0.013 # cull 25% of backgrounded stocker cattle in wellington county 
assume2$stomp$cull <- 0.009 # cull 25% of non-fed cattle in wellington county 
assume2$stomp$fnsh <- 0.030 # cull 25% of fed cattle in wellington county 

## SLAUGHTER CAPACITY REDUCTION
assume2$capacity <- 0.5*assume1$capacity
...
for (i in qtr) {
  ...
  bkgd_2["q_sup steer",  ii] <- bkgd_2["q_sup steer",  ii] + max(round(.95*assume2$dairy_fdr*fit(qq$bkgrd$dairy$data, logeq = F)$yhat)[i], 0)
  bkgd_2["q_sup heifer", ii] <- bkgd_2["q_sup heifer", ii] + max(round(.95*(1-assume2$dairy_fdr)*fit(qq$bkgrd$dairy$data, logeq = F)$yhat)[i], 0)
  ...
  ## SHOCK: CAPACITY CONSTRAINT FOR FINISHED CATTLE
  if (spare_capacity2 == 0) {
    
    # gender ratios
    temp1 <- fnsh_2["q_sup heifer", ii]/fnsh_2["q_sup total",  ii]
    temp2 <- fnsh_2["q_sup steer",  ii]/fnsh_2["q_sup total",  ii]
    temp3 <- fnsh_2["q_sup heifer", ii]/fnsh_2["q_sup total",  ii]
    temp4 <- fnsh_2["q_sup steer",  ii]/fnsh_2["q_sup total",  ii]
    
    # can't produce more than capacity constraint
    fnsh_2["q_sup total",  ii] <- fnsh_2["q_dem total",  ii] <- assume2$capacity
    
    # supply and demand by gender
    fnsh_2["q_sup heifer", ii] <- temp1*fnsh_2["q_sup total", ii]
    fnsh_2["q_sup steer",  ii] <- temp2*fnsh_2["q_sup total", ii]
    fnsh_2["q_dem heifer", ii] <- temp1*fnsh_2["q_dem total", ii]
    fnsh_2["q_dem steer",  ii] <- temp2*fnsh_2["q_dem total", ii]
  ...
  ## SHOCK -- NO PROCESSED BEEF MOVEMENT ACROSS BORDERS FOR TWO QUARTERS
  ...
  if (it < 3) {
    ...
    # if residual demand is negative, you have excess supply and processed beef CANNOT move out
    if (resid_dem <  0) proc_2["q_mip total", ii] <- 0
    
    # if residual demand is positive, you have excess demand and processed beef CAN come in from the U.S.
  ...
  # note: there are no transmissions across sectors for non-fed
  # base prices
  ...
  ## if slaughter capacity constraint exceeds non-fed cattle (i.e. demand = 0)
  ## need to restrict prices to 0
  if (spare_capacity2 == 0){
    
    cull_2["price bull",  ii] <- 0
    cull_2["price cow", ii]   <- 0
